...
export default function App() {
  ...
  useEffect(() => {
    testFetchCurrentStandings().then((results) => setNames(results));
  }, []);

  useEffect(() => {
    testFetchCurrentSeasonRaceResults().then((results) =>
  ...
  useEffect(() => {
    testGetLastFiveRaceResults().then((results) =>
      setLastFiveRaceResults(results)
    );
  }, [currentSeasonRaceResults, testGetLastFiveRaceResults]);

  useEffect(() => {
    testFetchEventList().then((results) => setEventList(results));
  }, []);
  ...
  }, [getNextCircuitIdAndType, lastFiveRaceResults]);
  ...
  useEffect(() => {
    testFetchNextTrackData(nextRace).then((results) =>
      setNextRaceHistory(results)
    );
  }, [nextRace, testFetchNextTrackData]);
  ...
  const racerData = driverTableData.map((driver) =>
    formatRow(
      driver.name,
